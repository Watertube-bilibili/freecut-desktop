...
0.4.2 可在“AI 语音 → 语音朗读”先选择模型下载父目录。软件自动建立 `FreeCut-VoiceModels` 子目录，将 ChatTTS 与轻量 AISHELL 中文朗读模型及其模型缓存放在那里；两种朗读引擎共用这一目录设置。Python、独立依赖环境、已生成音频和字幕识别模型仍在原位置，因此原数据目录仍需保留可用空间。

已完整安装的模型先复制并校验，成功后才启用新目录，源文件保留；失败则保持原配置。旧下载缓存和未完成文件保留原位，后续模型下载缓存使用新位置。下载或生成期间不允许切换，支持恢复默认并记住选择。切换不会自动释放原模型或旧缓存占用的空间，也不会移动已被工程引用的音频。操作范围见[模型目录说明](AI-MODELS.md#自定义朗读模型目录)，本次验收见 [0.4.2 验证记录](VERIFICATION-042.md)。

![实际桌面中生成、试听并加入时间线](screenshots/chattts.png)
...
- 默认数据保存在 Electron `userData/ai/chattts-0.2.5-cpu-v1`。便携版默认随应用的便携数据目录存放；0.4.2 另选目录时，只有朗读模型和模型缓存改用新位置，Python、依赖及生成音频保留原位置。
- 即使电脑已安装 Python，也使用独立托管 Python 与独立 venv，不写入系统 Python 的 site-packages。
- 运行环境、工具和临时文件使用应用管理的独立目录；朗读模型及其模型缓存遵循已选的目录设置。安装命令不修改用户 PATH。
