...
2. 语音朗读：打开“语音朗读”，可先选择朗读模型目录，再首次下载 AISHELL-3 或 ChatTTS；输入文本、选音色与语速、生成，试听后在播放头位置加入音频。
3. 下载或推理失败可以重试；处理中可以取消。0.3.0 的大文件按 4 MiB 分段请求，连接中断保留下载前缀供续传；完成后复验完整散列。校验失败会重新下载该文件，已校验完整文件复用。用户主动取消会清理当前未完成文件。
4. 默认数据位于 Electron `app.getPath('userData')/ai`。0.4.2 可另选朗读模型及其模型缓存的位置；运行环境、字幕识别模型与生成音频仍保留原位置。AISHELL 生成的朗读 WAV 保存在默认数据中的 `speech` 子目录，项目引用该文件，不要手工移动它。

## 自定义朗读模型目录

0.4.2 在“AI 语音 → 语音朗读”提供目录设置，中英文界面都可使用。选择一个父目录后，软件会自动创建并使用其中的 `FreeCut-VoiceModels` 文件夹，例如选择 `D:\Models` 后使用 `D:\Models\FreeCut-VoiceModels`，无需手工建立该子目录。

| 内容 | 是否跟随朗读模型目录 |
| --- | --- |
| 已完整安装的 ChatTTS / AISHELL 中文朗读模型、后续模型下载缓存 | 是；完整模型复制校验后启用，后续缓存写入新位置 |
| 原有下载缓存和未完成文件 | 否，保留原位置，不参与迁移 |
| ChatTTS 的 Python、独立依赖环境和 sherpa 运行环境 | 否，保留原位置 |
| SenseVoice / Whisper 自动字幕模型 | 否，保留原位置 |
| 已生成的 WAV、临时处理音频和工程引用 | 否，保留原位置 |

已完整安装的模型会先复制到目标位置并校验，成功后才切换配置；源文件保留。旧下载缓存和未完成文件不复制，后续朗读模型下载缓存写入新位置。复制、校验或配置保存失败时继续使用原目录，不把未完成的目标当作可用模型。复制期间需为新旧两份模型文件预留空间；切换成功也不会自动删除源文件或旧缓存，因此不会立刻回收旧目录的空间。

下载或生成进行时不能切换目录，完成或取消后再操作。可恢复默认目录，已保存的目录选择会跨次启动保留。便携版的默认数据仍在随应用的 `FreeCutData` 中，另选到其他磁盘的模型目录不会随便携程序自动移动；该磁盘需要保持可用。

此设置不迁移整个用户数据目录，也不更改下载来源或模型许可。模型和运行环境首次仍需下载；本次目录功能的实际测试结果见 [0.4.2 验证记录](VERIFICATION-042.md)，不能将下面的旧版推理测试等同于新目录已通过验收。
