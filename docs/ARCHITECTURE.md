...
flowchart LR
  ...
  M <--> P[Noise 加密连接]
  P <--> O[异地编辑器]
  Q[公开 HyperDHT 发现节点] -. 发现与 NAT 穿透尝试 .-> P
  M <--> R[高级 LAN 或可信 VPN 内的 HTTP 直连]
```
...
| `electron/collaboration-remote.cjs` | HyperDHT 公开发现、NAT 穿透尝试、Noise 加密流与固定本机房间服务连接、阶段与取消清理 |
...
0.4.1 默认使用异地协作。用户主动创建房间后，宿主启动仅绑定回环地址的本机房间服务，通过 HyperDHT 公开发现网络发布房间并尝试 NAT 穿透。`freecut2:` 邀请码封装房间公钥和访问凭据，不是本机 IP 的编码。伙伴粘贴邀请码后建立 Noise 加密连接，连接只承载固定的本机协作服务，不接受对方指定任意 TCP 目标或系统路径。用户无需另装 VPN、注册账号或自行部署服务器；公开发现节点仍是连接建立所依赖的外部基础设施。

远端模式在网络检查、房间发布和连接过程中上报不同阶段，仅实际成功后标记房间已开启或已加入。取消通过任务代次和 AbortController 中止待处理连接，并清理发现节点实例、加密流及本机套接字。房主需要持续在线；UDP 策略、NAT 类型或发现网络不可达都可能导致失败。软件没有媒体流量中继，不保证所有网络必能互通。公开节点可观察 IP、端口、时间等网络元数据，工程和媒体在 Noise 流中加密，不作为明文发送给发现节点；这不构成匿名保证。

高级“局域网 / IP 直连”继续使用 IPv4、端口（默认 45823）和房间密钥，兼容旧 `freecut1:` 邀请码。该模式不启动公网发现，采用带密钥鉴权但未加密的 HTTP，只用于已可达的可信局域网或可信加密 VPN。旧码不提供自动异地连接，不能将该 HTTP 端口直接暴露到公网。
...
网络工程经过字段白名单、引用和数值验证，不接受任意本地路径。共享媒体必须来自本机已授权并属于该工程素材清单的文件，按 SHA-256 和大小验证后流式上传／下载至各参与者的协作缓存，再注册本地媒体 URL；不会开放整个文件系统。离开房间保留本地工程与已下载素材，对方已下载的副本不会因房间结束而删除。上限为 8 位编辑者、256 个素材、工程 JSON 2 MiB、单文件 8 GiB、每房间 32 GiB、4 路并行素材传输；接收前检查磁盘空间并预留 256 MiB。房间关闭后旧密钥不能重新打开该房间。

使用步骤、旧码兼容和缓存保留见 [协作说明](COLLABORATION.md)。组件及许可见 [P2P 组件说明](third-party/p2p/README.md)；实际网络测试条件及发行边界见 [0.4.1 验证记录](VERIFICATION-041.md)，不能将同机或局域网测试表述为真实异地公网验收。
