...
import type { AnimProperty, Clip, MediaAsset, Project } from './types';
import {
  createProject,
  createClip,
  defaultEffects,
...
import { renderProject, syncAudio, clearMediaCache } from './core/renderer';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      const p = projectRef.current,
        t = timeRef.current;
      if (canvas.current && (lastProject !== p || lastTime !== t)) {
        lastProject = p;
        lastTime = t;
        try {
          await renderProject(canvas.current, p, t, {
  ...
  function switchLayout() {
    ...
    localStorage.setItem('freecut-layout', next ? 'mobile' : 'professional');
  ...
  async function save() {
    try {
      if (window.freecut) {
        const path = await window.freecut.saveProject(projectRef.current);
        if (path) notify('工程已保存。');
      } else {
        downloadFile(
          `${projectRef.current.name}.freecut`,
  ...
  async function open() {
    try {
      if (window.freecut) {
        const data = await window.freecut.openProject();
        if (data) replaceProject(validateProject(data));
  ...
  }, [selected, exportOpen, aiOpen, onboarding, remove, undo, redo, split, seek]);
  ...
  return (
    <div
      className={`app ${mobile ? 'mobile-mode' : ''} ${mobileShelf ? 'shelf-open' : ''} ${!inspectorOpen ? 'inspector-collapsed' : ''}`}
    >
      <header className="topbar">
        ...
        />
        <span className="local-badge">本地工程</span>
        <div className="top-actions">
          <button
            title="新建工程"
            onClick={() =>
              project.clips.length ? setNewConfirm(true) : replaceProject(createProject())
            }
          >
            <Plus size={16} />
            <span>新建</span>
              ...
              <>
                ...
                {assets.length ? (
                  ...
                  <div className="library-empty">
                    ...
                    <p>{search ? '换一个关键词试试。' : '点击导入，然后将素材拖入下方时间线。'}</p>
                  </div>
                )}
                {tab === 'audio' && (
                  <button
        ...
        <main className="preview-panel">
          <div className="preview-header">
            ...
            <div>
              ...
              >
                {!presets.some((p) => p.width === project.width && p.height === project.height) && (
      ...
      <section className="timeline-panel" aria-label="多轨时间线">
        <div className="timeline-toolbar">
          <div className="edit-actions">
            ...
            <button className="icon-button" title="复制片段" disabled={!clip} onClick={duplicate}>
              <Copy size={16} />
            </button>
            <button className="icon-button" title="删除 Delete" disabled={!clip} onClick={remove}>
              <Trash2 size={16} />
            </button>
            <i />
            <button
      ...
      <footer className="statusbar">
        <span>
          <span className="status-dot" />
          {window.freecut ? '桌面版' : '浏览器预览'} <span className="muted">0.1.0</span>
        </span>
        <span>
      ...
      <input
        ...
        onChange={async (e) => {
          try {
            ...
            if (file) replaceProject(validateProject(JSON.parse(await file.text())));
          } catch (err) {
            ...
            <p>
              {
                [
                  ...
                  '导入素材，拖到时间线，再点击右侧参数旁的菱形添加关键帧。自动字幕和语音朗读可按需下载模型。',
                ][tourStep]
              }
            </p>
      ...
      {newConfirm && (
        <div className="modal-backdrop">
          <div className="dialog" role="dialog" aria-modal="true" aria-label="新建工程">
            <h2>创建新的工程</h2>
            <p>请先保存当前工程。新工程也可以通过撤销回到上一状态。</p>
            <div className="dialog-actions">
              <button onClick={() => setNewConfirm(false)}>继续编辑</button>
              <button onClick={() => void save()}>保存当前工程</button>
              <button
                className="primary"
                onClick={() => {
