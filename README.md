...
导出会自动选择合适的处理路径。0.5.0 支持常规裁剪、拼接、图片和变速的 FFmpeg 原生路径，并将该路径扩展到符合条件的多层叠加、静态变换、位置动画及预先栅格化的文字 / 图形。超出原生支持范围的效果、动态缩放或旋转等仍使用预览共用的合成器，将 RGBA 画面直接送入编码器，边渲染边编码；不会忽略不支持的效果，也不会逐帧生成临时 PNG。实际提速取决于工程与设备；[0.3.2 验证记录](docs/VERIFICATION-032.md)中的旧测试不是 0.5.0 的性能数据。
