...
description: >
  ...
  borders, hard offset shadows (no blur), NotoSansSC uppercase at 0.92 line-height,
  NotoSansSC taxonomy, NotoSansSC body, the four-accent palette. Composition,
...
typography:
  ...
  body-lg:    { fontFamily: "NotoSansSC", px: 28, cqw: 1.46, weight: 400, lineHeight: 1.4 }
  body-md:    { fontFamily: "NotoSansSC", px: 24, cqw: 1.25, weight: 400, lineHeight: 1.3 }
  mono-label: { fontFamily: "NotoSansSC", px: 24, cqw: 1.25, weight: 400, tracking: "0.06em", upper: true }
  mono-kicker:{ fontFamily: "NotoSansSC", px: 24, cqw: 1.25, weight: 400, tracking: "0.14em", upper: true }
  table-head: { fontFamily: "NotoSansSC", px: 28, cqw: 1.46, weight: 400, upper: true }
  # — display / hero ramp (frame-native, cqw-first) —
  step-title: { fontFamily: "NotoSansSC", px: 34,  cqw: 1.77, weight: 400, lineHeight: 1.0, upper: true }
  badge-label:{ fontFamily: "NotoSansSC", px: 28,  cqw: 1.46, weight: 400, upper: true }
  marker:     { fontFamily: "NotoSansSC", cqw: 2.4, weight: 400, lineHeight: 1.0, upper: true }
  stamp-num:  { fontFamily: "NotoSansSC", cqw: 2.2, weight: 400, lineHeight: 0.9 }
  stat-num:   { fontFamily: "NotoSansSC", cqw: 6.4, weight: 400, lineHeight: 0.88 }
  step-num:   { fontFamily: "NotoSansSC", cqw: 7.0, weight: 400, lineHeight: 0.85 }
  display-head: { fontFamily: "NotoSansSC", cqw: 4.2, weight: 400, lineHeight: 0.92, tracking: "-0.01em", upper: true }
  display-lg: { fontFamily: "NotoSansSC", cqw: 8.0,  weight: 400, lineHeight: 0.9,  tracking: "-0.01em", upper: true }
  display-xl: { fontFamily: "NotoSansSC", cqw: 11.0, weight: 400, lineHeight: 0.9,  tracking: "-0.01em", upper: true }
  display-hero:{ fontFamily: "NotoSansSC", cqw: 15.5,weight: 400, lineHeight: 0.84, tracking: "-0.02em", upper: true }
...
- **Fonts** — already set to **NotoSansSC** (display) / **NotoSansSC** (body); ignore any preset font name lingering in prose.
...
The display voice is **NotoSansSC in strict uppercase at 0.92 line-height** — letters overlap
their own cap height; that tightness is the brand. **NotoSansSC** carries every label, kicker,
counter, and axis as a "technical artifact" register. **NotoSansSC** carries the few body lines.
...
- **NotoSansSC uppercase**, 0.92 line-height, always; sentence-case NotoSansSC does not exist.
...
- **Uppercase + 0.92 line-height on all NotoSansSC**, always. Mono carries 0.06–0.14em tracking. Never letter-space NotoSansSC beyond the encoded −0.01em; never set body centered.
...
**Fixed** NotoSansSC uppercase 0.84 lh, one accent on the wordmark, square corners. **Free** which
...
- Size headlines **fit-to-measure**; render NotoSansSC uppercase at 0.92 lh.

### Don't

- Don't round corners (except the pill chip); don't gradient, blur, or glow.
- Don't set NotoSansSC in sentence case or letter-space it beyond −0.01em.
- Don't use all four accents on one frame, a fifth accent, or pure white.
- Don't center body copy or set labels in anything but NotoSansSC.
...
- **Type** — NotoSansSC uppercase 0.92 lh, fit-to-measure, ≥1.4cqw on load-bearing lines.
...
- **NotoSansSC requires Google Fonts**; fallback is `sans-serif`. CJK pairing (Noto Serif SC 900 / NSC 400) carries over from the source's CJK section.
...
Bundled NotoSansSC-Regular.otf (400) and NotoSansSC-Bold.otf (700), OFL 1.1, replace all Western display/body/mono font families to guarantee real Chinese glyphs. Main Chinese display may use 700 at 100–150px, support at 400, 30–42px. Every frame declares both files in @font-face using project-relative assets/fonts paths. Mint (#78E4BC) and ink (#091C19) are principal; cream (#F4F3E8) creates chapter contrast. No external font requests.
